...
int main() {
    ifstream inputFile("input/task2.2.txt");
    if (!inputFile.is_open()) {
        cerr << "Loi: Khong the mo file input/task2.2.txt\n";
        return 1;
    }
    ...
    ofstream outputFile("output/task2.2.txt");
    if (outputFile.is_open()) {
        outputFile << "===== BAN ANH XA CUOI CUNG =====\n";
        for (char c = 'A'; c <= 'Z'; c++) {
            outputFile << c << " -> " << decryptMap[c] << "\n";
        }
        outputFile << "\n===== VAN BAN DA GIAI MA =====\n";
        outputFile << decryptText(ciphertext, decryptMap);
        outputFile.close();
        cout << "\nDa xuat ket qua giai ma thanh cong vao file output/task2.2.txt!\n";
    } else {
